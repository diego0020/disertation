...
\begin{itemize}
\item ARM \autocite{}: Used to \emph{standardize} data before fitting the linear model.
\item CAR \autocite{}: Used to fit the Anova model using type 3 sum of squares.
\irem randomForest \autocite{}: Used to fit a a random forest model to the data in order to calculate which variables are more effective at predicting the outcome of a given variable.
\end{itemize}
