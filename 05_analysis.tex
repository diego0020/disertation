...
The model represented in figure \ref{tab_features_solution} is a low level model, coupled with the current implementation. It should be used after the problem space model in order to define the best option to implement the required set of features. 

\begin{table}
\scriptsize
\input{figures/feature_ide/problem/table}	
\label{tab_features_problem}
\caption{Configurations of the current applications in relation to the problem space feature model( see figure \ref{fig_features_problem}).}
\end{table}
...
\label{tab_features_solution}
\caption{Configurations of the current applications in the solution space feature model( see figure \ref{fig_features_solution}).}
\end{table}
